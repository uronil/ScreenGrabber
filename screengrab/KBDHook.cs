...
namespace screengrab
{
    public static class KBDHook
    {
        ...
        static IntPtr HookProcFunction(int nCode, IntPtr wParam, [In] IntPtr lParam) {
            if (nCode == 0) {
                ...
                if (localHook) {
                    bool pressed = false;
                    if (lParam.ToInt64() >> 31 == 0)
                        pressed = true;

                    Key keys = (Key)wParam.ToInt32();
                    ...
                    bool pressed = false;
                    if (wParam.ToInt64() == 0x100 || wParam.ToInt32() == 0x104)
                        pressed = true;

                    Key keys = (Key)kbd.vkCode;
    ...
    public class LLKHEventArgs
    {
        ...
        public LLKHEventArgs(Key keys, bool pressed, uint time, uint scanCode) {
            this.keys = keys;
